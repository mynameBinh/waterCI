...
export default function AuthScreen({ onLoginSuccess }) {
  ...
  // Hiệu ứng logo lơ lửng (Bob animation) như trên Web
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.inner}>
          ...
          <View style={styles.card}>
            ...
            {error ? <View style={styles.msgError}><Text style={styles.msgErrorText}>❌ {error}</Text></View> : null}
            {success ? <View style={styles.msgSuccess}><Text style={styles.msgSuccessText}>✅ {success}</Text></View> : null}

            <View style={styles.field}>
              <Text style={styles.label}>Tên đăng nhập</Text>
              <TextInput
                style={styles.input} placeholder="Nhập username..."
                placeholderTextColor="#94a3b8" value={username} onChangeText={setUsername} autoCapitalize="none"
              />
            </View>
            ...
            <View style={styles.field}>
              ...
              <TextInput
                style={styles.input} placeholder="Nhập mật khẩu..."
                placeholderTextColor="#94a3b8" value={password} onChangeText={setPassword} secureTextEntry autoCapitalize="none"
              />
            </View>
            ...
            <TouchableOpacity style={styles.submitBtnWrapper} onPress={handleAuth} disabled={loading} activeOpacity={0.8}>
              <LinearGradient colors={['#67D8F5', '#0EA5E9']} style={styles.submitBtn}>
                {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitBtnText}>{isLogin ? 'Đăng nhập' : 'Đăng ký'}</Text>}
              </LinearGradient>
            </TouchableOpacity>

            <TouchableOpacity style={styles.toggleBtn} onPress={() => { setIsLogin(!isLogin); resetMessages(); }}>
              <Text style={styles.toggleText}>{isLogin ? 'Chưa có tài khoản? Đăng ký ngay' : 'Đã có tài khoản? Đăng nhập'}</Text>
            </TouchableOpacity>
          </View>
